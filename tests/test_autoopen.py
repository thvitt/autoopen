...
@pytest.mark.skipif(importlib.util.find_spec("zstandard") is None, reason="requires zstandard from pypi")
def test_read_zstd(test_path):
    return test_read(test_path, ".zst")


def test_read_unavailable(test_path):
    OpenHandler([".doesnotexist"], description='Not Existing format', module='doesnotexist').register()
